...
describe('Listing Users', () => {
  ...
  it('returns second page users and page indicator when page is set as 1', async () => {
    await addUsers(11);
    const response = await getUsers().query({ page: 1 }); // skip the route and access db directly
    expect(response.body.content[0].username).toBe('user11');
    expect(response.body.page).toBe(1);
  });
  it('returns first page when page is set below zero as request parameter', async () => {
    await addUsers(11);
    const response = await getUsers().query({ page: -5 }); // skip the route and access db directly
    expect(response.body.page).toBe(0);
  });
  it('returns 5 users and corresponding size indicator when size is set as 5 in request parameter', async () => {
    await addUsers(11);
    const response = await getUsers().query({ size: 5 }); // skip the route and access db directly
    expect(response.body.content.length).toBe(5);
    expect(response.body.size).toBe(5);
  });
  it('returns 10 users and corresponding size indicator when size is set as 1000', async () => {
    await addUsers(11);
    const response = await getUsers().query({ size: 1000 }); // skip the route and access db directly
    expect(response.body.content.length).toBe(10);
    expect(response.body.size).toBe(10);
  });
  it('returns 10 users and corresponding size indicator when size is set as 0', async () => {
    await addUsers(11);
    const response = await getUsers().query({ size: 0 }); // skip the route and access db directly
    expect(response.body.content.length).toBe(10);
    expect(response.body.size).toBe(10);
  });
  it('returns page as 0 and size as 10 when non numeric query params provided for both', async () => {
    await addUsers(11);
    const response = await getUsers().query({ size: 'size', page: 'page' }); // skip the route and access db directly
    expect(response.body.size).toBe(10);
    expect(response.body.page).toBe(0);
  });
});
...
describe('Get User', () => {
  ...
  it('returns only id,username and email in response body when an active user exists', async () => {
    const user = await User.create({
      username: 'user1',
      mail: 'user1@mail.com',
      inactive: false,
